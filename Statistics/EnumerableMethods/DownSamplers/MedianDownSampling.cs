...
public static partial class DownSampler
{
    ...
    public static double[] DownSampleMedian(IEnumerable<double> source, int targetLength)
    {
        // precondition checks
        if (targetLength < 0)
        ...
        MovingMedian_Double slidingAverageWindow = new((int)Math.Ceiling(factor));
        // down-sample
        int i = 0;
        int targetFill = 0;
    ...
    /// <returns></returns>
    /// <exception cref="ArgumentOutOfRangeException">the source array must be longer than the target array and targetLength must be > 1</exception>
    public static T[] DownSampleMedian<T>(IEnumerable<T> source, int targetLength) where T : INumber<T>
    {
        // precondition checks
        if (targetLength < 0)
            throw new ArgumentOutOfRangeException($"{nameof(targetLength)} must be >= 0!");
    ...
    /// <returns></returns>
    /// <exception cref="ArgumentOutOfRangeException">the source array must be longer than the target array and targetLength must be > 1</exception>
    public static decimal[] DownSampleMedian(IEnumerable<decimal> source, int targetLength)
    {
        // precondition checks
        if (targetLength < 0)
            throw new ArgumentOutOfRangeException($"{nameof(targetLength)} must be >= 0!");
        ...
        double factor = sourceLength / (double)targetLength;
        MovingMedian_Decimal slidingAverageWindow = new((int)Math.Ceiling(factor));
        // down-sample
        int i = 0;
        int targetFill = 0;
